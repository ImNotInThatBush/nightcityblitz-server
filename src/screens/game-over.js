...
export class GameOverScreen {
    constructor(changeStateCallback, audioManager, result) {
        ...
        this.isMultiplayer = result.isMultiplayer;
        this.resultText = result.playerWon ? "CONNECTION SECURED" : "CONNECTION LOST";
        this.button = { label: this.isMultiplayer ? "RETURN TO LOBBY" : "DISCONNECT", isHovered: false };
    }
    ...
    update(input) {
        ...
        if (this.button.isHovered && input.mouse.clicked) {
    ...
    draw(renderer) {
        ...
        renderer.drawHudButton(this.isMultiplayer ? '>>' : '<<', this.button.label, (Config.BASE_WIDTH - 300) / 2, Config.BASE_HEIGHT - 200, 300, 50, this.button.isHovered);
    }
}
